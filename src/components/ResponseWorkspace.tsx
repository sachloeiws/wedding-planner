      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        ...
        {!editingSetup && (
          <div className="flex gap-2">
            <button onClick={loadResponses} disabled={isLoading} className="px-3 py-2 rounded-xl bg-[#8E9E8C] text-white text-xs font-semibold flex items-center gap-1.5 disabled:opacity-60"><RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} />同步最新回覆</button>
            <button onClick={() => { setEditingSetup(true); setStep(1); }} className="px-3 py-2 rounded-xl border border-[#E2D9CD] text-[#8C745A] text-xs font-semibold"><Settings2 className="w-3.5 h-3.5 inline mr-1" />重新設定</button>
          </div>
        )}
      </div>
      ...
      {!editingSetup && (
        <>
          <section className="space-y-3">
            ...
            <div className="md:hidden space-y-2">{visibleGuests.map(guest => { const tone = attendanceTone(guest.attendance); return <details key={guest.id} className="p-3 rounded-xl bg-[#FAF8F5] border border-[#F0EBE4]"><summary className="list-none flex items-center justify-between"><div><p className="text-sm font-semibold">{guest.name}</p><p className="text-[10px] text-[#A6998A] mt-1">{guest.relationship || '未設定關係'}・{guest.count} 位</p></div><span className={`px-2 py-1 rounded-full text-[10px] ${tone.className}`}>{tone.label}</span></summary><div className="pt-3 mt-3 border-t text-xs text-[#8C745A] space-y-1"><p>聯絡：{guest.phone || guest.email || '—'}</p><p>備註：{guest.notes || '—'}</p></div></details>; })}</div>
