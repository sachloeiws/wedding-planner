...
export default function ResponseWorkspace(props: Props) {
  ...
  const standardizedGuests = useMemo(() => filterResponses(responses, filterRules).map(row => {
    const counts = getResponseGuestCounts(row, fieldMapping.countField);
    return {
    id: row.id,
    row,
  ...
  const totalAdults = standardizedGuests.reduce((sum, guest) => sum + guest.adultCount, 0);
  const totalChildren = standardizedGuests.reduce((sum, guest) => sum + guest.childCount, 0);
        ...
        <section className="bg-[#FAF8F5] border border-[#F0EBE4] rounded-[1.5rem] p-4 sm:p-5 space-y-5">
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="bg-white border border-[#F0EBE4] rounded-xl p-3"><p className="text-[10px] font-bold text-[#A6998A] mb-2">即時預覽</p><div className="grid sm:grid-cols-3 gap-2">{standardizedGuests.map(guest => <div key={guest.id} className="p-3 bg-[#FAF8F5] rounded-lg"><p className="text-xs font-semibold">{guest.name || '未辨識姓名'}</p><p className="text-[10px] text-[#A6998A] mt-1">{guest.relationship || '未設定關係'}・大人 {guest.adultCount} 位・兒童 {guest.childCount} 位</p></div>)}</div></div>
      ...
      {!editingSetup && (
        <>
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2"><div className="p-3 rounded-xl bg-[#FAF8F5] border border-[#F0EBE4]"><span className="text-[10px] text-[#A6998A]">賓客回覆</span><strong className="block font-mono text-lg">{standardizedGuests.length}</strong></div><div className="p-3 rounded-xl bg-[#FAF8F5] border border-[#F0EBE4]"><span className="text-[10px] text-[#A6998A]">預計席位</span><strong className="block font-mono text-lg">{standardizedGuests.reduce((sum, guest) => sum + guest.count, 0)}</strong><span className="block text-[9px] text-[#A6998A]">大人 {totalAdults}・兒童 {totalChildren}</span></div><div className="p-3 rounded-xl bg-emerald-50 border border-emerald-100"><span className="text-[10px] text-emerald-700">明確出席</span><strong className="block font-mono text-lg text-emerald-700">{standardizedGuests.filter(guest => /是|會|出席|參加|可以|yes/i.test(guest.attendance)).length}</strong></div><div className="p-3 rounded-xl bg-amber-50 border border-amber-100"><span className="text-[10px] text-amber-700">需確認</span><strong className="block font-mono text-lg text-amber-700">{standardizedGuests.filter(guest => !guest.attendance).length}</strong></div></div>
            ...
            <div className="hidden md:block border border-[#F0EBE4] rounded-2xl overflow-hidden"><table className="w-full text-xs"><thead className="bg-[#FAF8F5] text-[#8C745A]"><tr><th className="text-left p-3">姓名</th><th className="text-left p-3">出席</th><th className="text-left p-3">大人</th><th className="text-left p-3">兒童</th><th className="text-left p-3">合計</th><th className="text-left p-3">關係／群組</th><th className="text-left p-3">聯絡方式</th><th className="text-left p-3">備註</th></tr></thead><tbody>{visibleGuests.map(guest => { const tone = attendanceTone(guest.attendance); return <tr key={guest.id} className="border-t border-[#F0EBE4]"><td className="p-3 font-semibold">{guest.name}</td><td className="p-3"><span className={`px-2 py-1 rounded-full text-[10px] ${tone.className}`}>{tone.label}</span></td><td className="p-3 font-mono">{guest.adultCount}</td><td className="p-3 font-mono">{guest.childCount}</td><td className="p-3 font-mono font-semibold">{guest.count}</td><td className="p-3">{guest.relationship || '—'}</td><td className="p-3"><div>{guest.phone || guest.email || '—'}</div></td><td className="p-3 max-w-[220px] truncate" title={guest.notes}>{guest.notes || '—'}</td></tr>; })}</tbody></table></div>
            <div className="md:hidden space-y-2">{visibleGuests.map(guest => { const tone = attendanceTone(guest.attendance); return <details key={guest.id} className="p-3 rounded-xl bg-[#FAF8F5] border border-[#F0EBE4]"><summary className="list-none flex items-center justify-between gap-2"><div className="min-w-0"><p className="text-sm font-semibold truncate">{guest.name}</p><p className="text-[10px] text-[#A6998A] mt-1 truncate">{guest.relationship || '未設定關係'}・大人 {guest.adultCount} 位・兒童 {guest.childCount} 位</p></div><span className={`shrink-0 max-w-[45%] truncate px-2 py-1 rounded-full text-[10px] ${tone.className}`}>{tone.label}</span></summary><div className="pt-3 mt-3 border-t text-xs text-[#8C745A] space-y-1 break-words"><p>聯絡：{guest.phone || guest.email || '—'}</p><p>備註：{guest.notes || '—'}</p></div></details>; })}</div>
